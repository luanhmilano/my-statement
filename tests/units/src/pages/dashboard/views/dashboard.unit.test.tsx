...
vi.mock('@/pages/dashboard/components/side-menu', () => ({
  default: ({ logout, activeView, onNavigate, isMobile, isOpen, onClose }: any) => (
    <div 
...
describe('DashboardView', () => {
  ...
  describe('Mobile vs Desktop layout', () => {
    ...
    it('renders mobile layout when isMobile is true', () => {
      render(<DashboardView {...mockProps} isMobile={true} isMobileMenuOpen={true} />);
  ...
  describe('CSS classes', () => {
    it('applies correct CSS classes to elements', () => {
      ...
      expect(screen.getByTestId('header').closest('.mainContent')).toBeInTheDocument();
      expect(screen.getByTestId('home-view').closest('.main')).toBeInTheDocument();
    });

    it('applies sideMenu class for desktop layout', () => {
      const { container } = render(<DashboardView {...mockProps} isMobile={false} />);
      
      expect(container.querySelector('.sideMenu')).toBeInTheDocument();
    });

    it('does not apply sideMenu class wrapper for mobile layout', () => {
      const { container } = render(<DashboardView {...mockProps} isMobile={true} />);
