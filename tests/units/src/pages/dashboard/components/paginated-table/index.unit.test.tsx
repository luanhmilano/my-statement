...
const generateMockData = (count: number): StatementItem[] => {
  return Array.from({ length: count }, (_, index) => ({
    ...
    type: (index % 2 === 0 ? 'Withdrawal' : 'Deposit') as
      | 'Withdrawal'
      | 'Deposit',
...
describe('PaginatedTable', () => {
  ...
  describe('Table Rendering', () => {
    it('renders table with correct headers', () => {
      ...
      expect(
        screen.getByRole('columnheader', { name: 'Card' })
      ).toBeInTheDocument();
      expect(
      ...
      expect(
        screen.getByRole('columnheader', { name: 'Receipt' })
      ).toBeInTheDocument();
    });
  ...
  describe('Page Numbers', () => {
    ...
    it('navigates to specific page when page number is clicked', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('Showing 21-30 of 50 results')
        ).toBeInTheDocument();
      });
  ...
  describe('Edge Cases', () => {
    ...
    it('handles exactly 11 items (pagination needed)', () => {
      ...
      expect(screen.getByText('Next')).toBeInTheDocument();
      expect(
        screen.getByText('Showing 1-10 of 11 results')
      ).toBeInTheDocument();
    });
  ...
  describe('Data Updates', () => {
    it('resets to first page when data changes', async () => {
      const { rerender } = render(
        <PaginatedTable data={generateMockData(25)} />
      );
